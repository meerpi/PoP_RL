...
#define RL_ACTION_NONE 0
#define RL_ACTION_LEFT 1
#define RL_ACTION_RIGHT 2
#define RL_ACTION_UP 3
#define RL_ACTION_DOWN 4
#define RL_ACTION_SHIFT 5

int rl_mode = 0;
int rl_visual_mode = 0;
int rl_action = 0;
int rl_kid_dead = 0; // Set to 1 when Kid dies, Python should reset

// Save a frame to a file if needed
void rl_save_frame(const char *filename) {
...
void play_level(int level_number) {
  ...
  for (;;) {
    ...
    if (level_number != current_level) {
      ...
      cutscene_func =
          tbl_cutscenes[custom->tbl_cutscenes_by_index[level_number]];
    ...
    // busy waiting?
    while (check_sound_playing() && !do_paused())
      idle();
    stop_sounds();
#ifdef USE_REPLAY
    if (replaying)
...
void do_startpos() {
  ...
  } else if (/*current_level == 13*/ custom->tbl_entry_pose[current_level] ==
             2) {
...
void set_start_pos() {
  ...
  droppedout = 0;
  play_seq();
  if (current_level == /*7*/ custom->falling_entry_level &&
      Char.room == /*17*/ custom->falling_entry_room) {
...
void redraw_screen(int drawing_different_room) {
  ...
  if (is_blind_mode) {
    ...
    if (curr_guard_color) {
      ...
      set_chtab_palette(chtab_addrs[id_chtab_5_guard],
                        &guard_palettes[0x30 * curr_guard_color - 0x30], 0x10);
    }
    need_drects = 0;
    redraw_room();
#ifdef USE_LIGHTING
    ...
    if (current_level == 15) {
      ...
      for (short i = 0; i < 14; ++i) {
        if (copyprot_room[i] == drawn_room) {
          set_curr_pos((copyprot_tile[i] % 10 << 5) + 24,
                       copyprot_tile[i] / 10 * 63 + 38);
    ...
    need_drects = 1;
    if (curr_guard_color) {
      // set_pal_arr(0x80, 0x10, &guard_palettes[0x30 * curr_guard_color -
      // 0x30], 1);
    }
    if (upside_down) {
...
void test_timings(test_timing_state_type *state) {
  ...
  if (!state->already_had_first_frame) {
    ...
    state->seconds_left_at_level_start =
        (1.0f / 60.0f) * (5 * state->ticks_left_at_level_start);
    printf("Seconds left = %f\n", state->seconds_left_at_level_start);
    state->already_had_first_frame = true;
  } else if (rem_tick % 12 == 11) {
    Uint64 current_counter = SDL_GetPerformanceCounter();
    float actual_seconds_elapsed =
        (float)(current_counter - state->level_start_counter) /
        (float)SDL_GetPerformanceFrequency();

    int ticks_left = (rem_min - 1) * 720 + rem_tick;
    float game_seconds_left = (1.0f / 60.0f) * (5 * ticks_left);
    float game_seconds_elapsed =
        state->seconds_left_at_level_start - game_seconds_left;

    printf("rem_min: %d   game elapsed (s): %.2f    actual elapsed (s): %.2f   "
           "  delta: %.2f\n",
           rem_min, game_seconds_elapsed, actual_seconds_elapsed,
           actual_seconds_elapsed - game_seconds_elapsed);
  }
}
#endif
...
int play_level_2() {
  ...
  while (1) { // main loop
    ...
    hitp_delta = 0;
    timers();
    // RL step mode: decode action from rl_action global
    if (rl_mode) {
      // Set raw control inputs using ABSOLUTE directions (LEFT=-1, RIGHT=+1).
      // user_control() in seg006.c calls flip_control_x() to convert these to
      // relative FORWARD/BACKWARD based on kid's facing direction.
      control_x = 0;
      control_y = 0;
      control_shift = 0;
      switch (rl_action) {
      case 0: // no-op
        break;
      case 1: // right
        control_x = 1;   // CONTROL_HELD_RIGHT
        break;
      case 2: // left
        control_x = -1;  // CONTROL_HELD_LEFT
        break;
      case 3: // up (jump / climb)
        control_y = -1;  // CONTROL_HELD_UP
        break;
      case 4: // down (crouch / descend)
        control_y = 1;   // CONTROL_HELD_DOWN
        break;
      case 5: // shift (grab / attack)
        control_shift = -1;  // CONTROL_HELD
        break;
      case 6: // right + up
        control_x = 1;
        control_y = -1;
      ...
      case 10: // right + shift
        ...
        control_shift = -1;
        break;
      case 11: // right + up + shift
        control_x = 1;
        control_y = -1;
        control_shift = -1;
        break;
      case 12: // right + down + shift
        control_x = 1;
        control_y = 1;
        control_shift = -1;
        break;
      case 13: // left + shift
        ...
        control_shift = -1;
        break;
      case 14: // left + up + shift
        control_x = -1;
        control_y = -1;
        control_shift = -1;
        break;
      case 15: // left + down + shift
        control_x = -1;
        control_y = 1;
        control_shift = -1;
        break;
    ...
    // RL step mode: return after one frame
    if (rl_mode) {
      if (rl_visual_mode) {
        draw_game_frame();
        update_screen();
      ...
      // Check if Kid is dead and auto-reset
      if (Kid.alive >= 0) {
        rl_kid_dead = 1;
        // Auto reset: restore Kid to start position
        hitp_curr = hitp_max = hitp_beg_lev;
        load_level();
        pos_guards();
        do_startpos();
        if (rl_visual_mode) {
...
#ifdef USE_REPLAY
    // At the exact "end of level" frame, preserve the seed to ensure
    // reproducibility, regardless of how long the sound is still playing
    // *after* this frame (torch animation modifies the seed!)
    if (keep_last_seed == 1) {
    ...
    if (is_restart_level) {
      ...
      if (next_level == current_level || check_sound_playing()) {
        ...
        if (keep_last_seed == -1) {
          random_seed =
              preserved_seed; // Ensure reproducibility in the next level.
          keep_last_seed = 0;
        }
#endif
...
void redraw_at_char2() {
  ...
  if (char_frame < frame_78_jumphang || char_frame >= frame_80_jumphang) {
    ...
    if (char_frame >= frame_137_climbing_3 &&
        char_frame < frame_145_climbing_11) {
      redraw_func = &set_redraw_floor_overlay;
    } else {
      // frames 102..106: fall
      if (char_action != actions_2_hang_climb &&
          char_action != actions_3_in_midair &&
          char_action != actions_4_in_freefall &&
          char_action != actions_6_hang_straight &&
          (char_action != actions_5_bumped ||
           char_frame < frame_102_start_fall_1 ||
           char_frame > frame_106_fall)) {
        return;
      }
    }
  }
...
void timers() {
  ...
  if (fixes->fix_quicksave_during_feather) {
    if (is_feather_fall > 0) {
      ...
      if (is_feather_fall == 0) {
        ...
        // rem_tick); printf("length = %d ticks\n", is_feather_fall);
#ifdef USE_REPLAY
        if (recording)
          special_move = MOVE_EFFECT_END;
#endif
      }
    }
  } else {
    ...
    if (is_feather_fall && (!check_sound_playing() || is_feather_fall > 225)) {
      ...
      if (recording)
        special_move = MOVE_EFFECT_END;
      if (!replaying) // during replays, feather effect gets cancelled in
                      // do_replay_move()
#endif
        is_feather_fall = 0;
    }
  ...
  if (current_level == /*8*/ custom->mouse_level &&
      Char.room == /*16*/ custom->mouse_room && leveldoor_open) {
    ++leveldoor_open;
  ...
  if (fixes->enable_super_high_jump && super_jump_timer > 0) {
    ...
    if (super_jump_timer == 0 && Kid.frame == frame_79_jumphang) {
      if (get_tile(super_jump_room, super_jump_col, super_jump_row) ==
              tiles_11_loose &&
          (curr_room_tiles[curr_tilepos] & 0x20) == 0) {
        make_loose_fall(1); // knocks the true loose tile above
        do_knock(super_jump_room,
                 super_jump_row); // shakes the rest of loose tiles
        ...
        do_knock(super_jump_room,
                 super_jump_row); // shakes loose tiles in the row
...
void check_mirror_image() {
  short xpos =
      x_bump[Char.curr_col + FIRST_ONSCREEN_COLUMN] +
      10; // I think 10 is the offset for mirror collision within a tile
...
void pos_guards() {
  for (short room1 = 0; room1 < ROOMCOUNT; ++room1) {
    ...
    if (guard_tile < 30) {
      level.guards_x[room1] =
          x_bump[(guard_tile % 10) + FIRST_ONSCREEN_COLUMN] + TILE_SIZEX;
...
void check_can_guard_see_kid() {
  ...
  */
  ...
  if (Guard.charid == charid_24_mouse) {
    // If the prince is fighting a guard, and the player does a quickload to a
    // state where the prince is near the mouse, the prince would draw the
    // sword. The following line prevents this.
    can_guard_see_kid = 0;
    return;
  }
  if ((Guard.charid != charid_1_shadow || current_level == 12) &&
      ...
      kid_frame != 0 &&
      (kid_frame < frame_219_exit_stairs_3 || kid_frame >= 229) &&
      Guard.direction != dir_56_none && Kid.alive < 0 && Guard.alive < 0 &&
      Kid.room == Guard.room && Kid.curr_row == Guard.curr_row) {
    can_guard_see_kid = 2;
    ...
    if (fixes->fix_doortop_disabling_guard) {
      // When the kid is hanging on the right side of a doortop, Kid.curr_col
      // points at the doortop tile and a guard on the left side will see the
      // prince. This fixes that.
      if (Kid.action == actions_2_hang_climb ||
          Kid.action == actions_6_hang_straight)
        left_pos += TILE_SIZEX;
    }
#endif
    // printf("Kid.curr_col = %d, Kid.action = %d\n", Kid.curr_col, Kid.action);
    short right_pos =
        x_bump[Guard.curr_col + FIRST_ONSCREEN_COLUMN] + TILE_MIDX;
...
#ifdef FIX_DOORTOP_DISABLING_GUARD
        || (fixes->fix_doortop_disabling_guard &&
            (get_tile_at_kid(right_pos) == tiles_7_doortop_with_floor ||
             get_tile_at_kid(right_pos) == tiles_12_doortop))
#endif
    ) {
    ...
    if (right_pos >= left_pos) {
      while (left_pos <= right_pos) {
        ...
        if (curr_tile2 == tiles_11_loose || curr_tile2 == tiles_18_chomper ||
            (curr_tile2 == tiles_4_gate &&
             curr_room_modif[curr_tilepos] < 112) ||
