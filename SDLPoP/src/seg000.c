...
void pop_main() {
  ...
  static char *fake_argv[] = {"./prince", "-nosound", "-megahit", NULL};
...
// up
  ...
  if (check_param("mute"))
    is_sound_on = 0;
  turn_sound_on_off((is_sound_on != 0) *
                    15); // Turn off sound/music if those options were set.

#ifdef USE_REPLAY
  if (g_argc > 1) {
    char *filename =
        g_argv[1]; // file dragged on top of executable or double clicked
    char *e = strrchr(filename, '.');
    if (e != NULL && strcasecmp(e, ".P1R") ==
                         0) { // valid replay filename passed as first arg
  ...
  // Initialize everything before load_mod_options() so it can show an error
  // dialog if needed.
  ...
  // I moved this after init_copyprot_dialog(), so open_dat() can show an error
  // dialog if needed.
...
void start_game() {
  ...
  for (word pos = 0; pos < 14; ++pos) {
    ...
    } while (entry_used[which_entry] ||
             letts_used[copyprot_letter[which_entry] - 'A']);
    cplevel_entr[pos] = which_entry;
    entry_used[which_entry] = 1;
    letts_used[copyprot_letter[which_entry] - 'A'] = 1;
  }
#endif
  if (custom->skip_title) { // CusPop option: skip the title sequence (level
                            // loads instantly)
...
const char *get_quick_path(char *custom_path_buffer, size_t max_len) {
  return get_writable_file_path(custom_path_buffer, max_len,
                                quick_file /*QUICKSAVE.SAV*/);
}
...
int quick_save(void) {
  ...
  const char *path =
      get_quick_path(custom_quick_path, sizeof(custom_quick_path));
...
int quick_load(void) {
  ...
  if (quick_fp != NULL) {
    ...
    if (enable_quicksave_penalty &&
        ...
        (current_level < /*13*/ custom->victory_stops_time_level ||
         (current_level == /*13*/ custom->victory_stops_time_level &&
          leveldoor_open < 2))) {
      int ticks_elapsed =
          720 * (rem_min - old_rem_min) + (rem_tick - old_rem_tick);
      // don't restore time at all if the elapsed time is between 0 and 1
      // minutes
      if (ticks_elapsed > 0 && ticks_elapsed < 720) {
        ...
        if (rem_min == 6)
          rem_tick = 719; // crop to "5 minutes" exactly, if hitting the
                          // threshold in <1 minute
        if (rem_min > 5 /*be lenient, not much time left*/ ||
            rem_min < 0 /*time runs 'forward' if < 0*/) {
          --rem_min;
        }
      }
    }
...
void check_quick_op() {
  if (!enable_quicksave)
    return;
  if (need_quick_save) {
    if ((!is_feather_fall || fixes->fix_quicksave_during_feather) &&
        quick_save()) {
  ...
  if (need_quick_load) {
    ...
    #ifdef USE_REPLAY
                    if (recording) {
                            stop_recording(); // quickloading would mess up the
    replay!
                    }
    #endif
    */
...
// ---- RL In-Memory Checkpoint System ----
// Reuses the quick_process() serialization pattern but writes to a memory buffer
// instead of a file. Used by the RL training loop for checkpoint curriculum.

#define RL_CHECKPOINT_BUF_SIZE 16384  // 16KB, more than enough for all game state
static char rl_checkpoint_buf[RL_CHECKPOINT_BUF_SIZE];
static int  rl_checkpoint_pos = 0;
static int  rl_checkpoint_valid = 0;

static int rl_process_save_mem(void *data, size_t data_size) {
    if (rl_checkpoint_pos + (int)data_size > RL_CHECKPOINT_BUF_SIZE) return 0;
    memcpy(rl_checkpoint_buf + rl_checkpoint_pos, data, data_size);
    rl_checkpoint_pos += (int)data_size;
    return 1;
}

static int rl_process_load_mem(void *data, size_t data_size) {
    if (rl_checkpoint_pos + (int)data_size > RL_CHECKPOINT_BUF_SIZE) return 0;
    memcpy(data, rl_checkpoint_buf + rl_checkpoint_pos, data_size);
    rl_checkpoint_pos += (int)data_size;
    return 1;
}

#ifdef USE_QUICKSAVE
int rl_save_checkpoint(void) {
    rl_checkpoint_pos = 0;
    int ok = quick_process(rl_process_save_mem);
    rl_checkpoint_valid = ok;
    return ok;
}

int rl_load_checkpoint(void) {
    if (!rl_checkpoint_valid) return 0;
    rl_checkpoint_pos = 0;
    int ok = quick_process(rl_process_load_mem);
    if (ok) {
        restore_room_after_quick_load();
    }
    return ok;
...
int rl_checkpoint_is_valid(void) {
    return rl_checkpoint_valid;
}
#else
// Fallback stubs if quicksave is not compiled in
int rl_save_checkpoint(void)     { return 0; }
int rl_load_checkpoint(void)     { return 0; }
int rl_checkpoint_is_valid(void) { return 0; }
#endif
// ---- End RL Checkpoint System ----
...
int process_key() {
  ...
  if (rem_min != 0 && Kid.alive > 6 &&
      (control_shift || key == SDL_SCANCODE_RETURN)) {
  ...
  */
  ...
  case SDL_SCANCODE_C | WITH_CTRL: // Ctrl+C
  {
    ...
    snprintf(sprintf_temp, sizeof(sprintf_temp),
             "SDL COMP v%u.%u.%u LINK v%u.%u.%u", verc.major, verc.minor,
             verc.patch, verl.major, verl.minor, verl.patch);
    answer_text = sprintf_temp;
    need_show_text = 1;
  } break;
  case SDL_SCANCODE_L | WITH_SHIFT: // Shift+L
    if (current_level < custom->shift_L_allowed_until_level /* 4 */ ||
        cheats_enabled) {
  ...
  if (cheats_enabled) {
    ...
    case SDL_SCANCODE_C: // c
      snprintf(sprintf_temp, sizeof(sprintf_temp), "S%d L%d R%d A%d B%d",
               drawn_room, room_L, room_R, room_A, room_B);
      answer_text = /*&*/ sprintf_temp;
      need_show_text = 1;
      break;
    case SDL_SCANCODE_C | WITH_SHIFT: // Shift+C
      snprintf(sprintf_temp, sizeof(sprintf_temp), "AL%d AR%d BL%d BR%d",
               room_AL, room_AR, room_BL, room_BR);
      answer_text = /*&*/ sprintf_temp;
      need_show_text = 1;
      break;
    ...
    case SDL_SCANCODE_N: // N --> view room below
      ...
      next_room = room_B;
      break;
    case SDL_SCANCODE_B | WITH_CTRL: // Ctrl+B --> Go back to the room where the
...
void play_frame() {
  ...
  if (fixes->fix_quicksave_during_feather && is_feather_fall >= 10 &&
      !check_sound_playing()) {
  ...
  } else if (/*current_level == 12*/ custom->tbl_seamless_exit[current_level] >=
             0) {
    // Special event: level 12 running exit
    if (Kid.room == /*23*/ custom->tbl_seamless_exit[current_level]) {
      ++next_level;
      // Sounds must be stopped, because play_level_2() checks next_level only
...
void load_sounds(int first, int last) {
  ...
  for (short current = first; current <= last; ++current) {
    if (sound_pointers[current] != NULL)
      continue;
    /*if (demo_mode) {
            sound_pointers[current] = decompress_sound((sound_buffer_type*)
    load_from_opendats_alloc(current + 10000)); } else*/
    {
      // sound_pointers[current] = (sound_buffer_type*)
      // load_from_opendats_alloc(current + 10000, "bin", NULL, NULL);
      // printf("overwriting sound_pointers[%d] = %p\n", current,
      // sound_pointers[current]);
...
// data:03BA
const char *const tbl_guard_dat[] = {"GUARD.DAT", "FAT.DAT", "SKEL.DAT",
...
void load_lev_spr(int level) {
  ...
  draw_rect(&screen_rect, color_0_black);
  free_optsnd_chtab();
  snprintf(filename, sizeof(filename), "%s%s.DAT", tbl_envir_gr[graphics_mode],
  ...
  if (guardtype != -1) {
    if (guardtype == 0) {
      dathandle = open_dat(custom->tbl_level_type[current_level] ? "GUARD1.DAT"
                                                                 : "GUARD2.DAT",
                           'G');
    }
    load_chtab_from_file(id_chtab_5_guard, 750, tbl_guard_dat[guardtype],
                         1 << 8);
    if (dathandle) {
      close_dat(dathandle);
    }
...
void load_level() {
  ...
  load_from_opendats_to_area(current_level + 2000, &level, sizeof(level),
                             "bin");
  close_dat(dathandle);
...
void reset_level_unused_fields(bool loading_clean_level) {
  ...
  // level.used_rooms is 25 on some levels. Limit it to the actual number of
  // rooms.
  if (level.used_rooms > ROOMCOUNT)
    level.used_rooms = ROOMCOUNT;

  // For these fields, only use the bits that are actually used, and set the
  // rest to zero. Good for repurposing the unused bits in the future.
  for (int i = 0; i < level.used_rooms; ++i) {
    // level.guards_dir[i]   &= 0x01; // 1 bit in use
    level.guards_skill[i] &= 0x0F; // 4 bits in use
  }

  // In savestates, additional information may be stored (e.g. remembered guard
  // hp) - should not reset this then!
  if (loading_clean_level) {
    for (int i = 0; i < level.used_rooms; ++i) {
      level.guards_color[i] &= 0x0F; // 4 bits in use (other 4 bits repurposed
                                     // as remembered guard hp)
    }
  }
}
...
void check_the_end() {
  if (next_room != 0 && next_room != drawn_room) {
    drawn_room = next_room;
    load_room_links();
    if (current_level == /*14*/ custom->win_level &&
        drawn_room == /*5*/ custom->win_room) {
#ifdef USE_REPLAY
      if (recording)
        stop_recording();
      if (replaying)
...
void check_fall_flo() {
  ...
  if (current_level == /*13*/ custom->loose_tiles_level &&
      (drawn_room == /*23*/ custom->loose_tiles_room_1 ||
       drawn_room == /*16*/ custom->loose_tiles_room_2)) {
    curr_room = room_A;
...
#define DEGREES_TO_RADIANS (M_PI / 180.0)
  ...
  // In this case, dist_squared is calculated as -32768 * -32768 + -32768 *
  // -32768 = 2147483648. But dist_squared is a 32-bit signed integer, which
  // cannot store that number, so it overflows to -2147483648. Therefore,
  // dist_squared < joystick_threshold*joystick_threshold becomes true, and the
  // game thinks the stick is centered. To fix this, we cast both sides of the
  // comparison to an unsigned 32-bit type.
  if ((dword)dist_squared < (dword)(joystick_threshold * joystick_threshold)) {
    ...
    double angle = atan2(
        raw_y,
        raw_x); // angle of the joystick: 0 = right, >0 = downward, <0 = upward
    // printf("Joystick angle is %f degrees\n", angle/DEGREES_TO_RADIANS);

    if (fabs(angle) <
        (60 * DEGREES_TO_RADIANS)) // 120 degree range facing right
      axis_state[0] = 1;

    else if (fabs(angle) >
             (120 * DEGREES_TO_RADIANS)) // 120 degree range facing left
      axis_state[0] = -1;

    else {
      // joystick is neutral horizontally, so the control should be released
      // however: prevent stop running if the Kid was already running / trying
      // to do a running-jump (this tweak makes it a bit easier to do (multiple)
      // running jumps)
    ...
    if (angle < (-30 * DEGREES_TO_RADIANS) &&
        angle > (-150 * DEGREES_TO_RADIANS)) // 120 degree range facing up
      axis_state[1] = -1;

    // down slightly less sensitive than up (prevent annoyance when your thumb
    // slips down a bit by accident) (not sure if this adjustment is really
    // necessary)
    else if (angle > (35 * DEGREES_TO_RADIANS) &&
             angle < (145 * DEGREES_TO_RADIANS)) // 110 degree range facing down
      axis_state[1] = 1;

    else {
      // joystick is neutral vertically, so the control should be released
      // however: should prevent unintended standing up when attempting to
      // crouch-hop
      if (!((Kid.frame >= frame_108_fall_land_2 &&
             Kid.frame <= frame_112_stand_up_from_crouch_3) &&
...
void read_joyst_control() {
  ...
  if (joystick_only_horizontal) {
    get_joystick_state_hor_only(joy_axis_ptr[SDL_CONTROLLER_AXIS_LEFTX],
                                joy_left_stick_states);
    get_joystick_state_hor_only(joy_axis_ptr[SDL_CONTROLLER_AXIS_RIGHTX],
                                joy_right_stick_states);
  } else {
    get_joystick_state(joy_axis_ptr[SDL_CONTROLLER_AXIS_LEFTX],
                       joy_axis_ptr[SDL_CONTROLLER_AXIS_LEFTY],
                       joy_left_stick_states);
    get_joystick_state(joy_axis_ptr[SDL_CONTROLLER_AXIS_RIGHTX],
                       joy_axis_ptr[SDL_CONTROLLER_AXIS_RIGHTY],
                       joy_right_stick_states);
  }

  if (joy_left_stick_states[0] == -1 || joy_right_stick_states[0] == -1 ||
      joy_button_states[JOYINPUT_DPAD_LEFT] & key_state)
    control_x = CONTROL_HELD_LEFT;

  if (joy_left_stick_states[0] == 1 || joy_right_stick_states[0] == 1 ||
      joy_button_states[JOYINPUT_DPAD_RIGHT] & key_state)
    control_x = CONTROL_HELD_RIGHT;

  if (joy_left_stick_states[1] == -1 || joy_right_stick_states[1] == -1 ||
      joy_button_states[JOYINPUT_DPAD_UP] & key_state ||
      joy_button_states[JOYINPUT_Y] & key_state)
    control_y = CONTROL_HELD_UP;

  if (joy_left_stick_states[1] == 1 || joy_right_stick_states[1] == 1 ||
      joy_button_states[JOYINPUT_DPAD_DOWN] & key_state ||
...
void draw_kid_hp(short curr_hp, short max_hp) {
  for (short drawn_hp_index = curr_hp; drawn_hp_index < max_hp;
       ++drawn_hp_index) {
    // empty HP
    method_6_blit_img_to_scr(get_image(id_chtab_2_kid, 217), drawn_hp_index * 7,
                             194, blitters_0_no_transp);
  }
  for (short drawn_hp_index = 0; drawn_hp_index < curr_hp; ++drawn_hp_index) {
    // full HP
    method_6_blit_img_to_scr(get_image(id_chtab_2_kid, 216), drawn_hp_index * 7,
                             194, blitters_0_no_transp);
  }
}
...
void draw_guard_hp(short curr_hp, short max_hp) {
  ...
  if (guard_charid != charid_4_skeleton && guard_charid != charid_24_mouse &&
      // shadow has HP only on level 12
      (guard_charid != charid_1_shadow || current_level == 12)) {
    for (short drawn_hp_index = curr_hp; drawn_hp_index < max_hp;
         ++drawn_hp_index) {
      method_6_blit_img_to_scr(chtab_addrs[id_chtab_5_guard]->images[0],
                               314 - drawn_hp_index * 7, 194, blitters_9_black);
    }
    for (short drawn_hp_index = 0; drawn_hp_index < curr_hp; ++drawn_hp_index) {
      method_6_blit_img_to_scr(chtab_addrs[id_chtab_5_guard]->images[0],
                               314 - drawn_hp_index * 7, 194,
                               blitters_0_no_transp);
    }
  }
}
...
// seg000:1200
void set_health_life() { hitp_delta = hitp_max - hitp_curr; }
...
void draw_hp() {
  ...
  bool blink_state =
      fixes->fix_one_hp_stops_blinking ? global_blink_state : rem_tick & 1;
#else
...
void do_delta_hp() {
  ...
  if (Opp.charid == charid_1_shadow && current_level == 12 &&
      guardhp_delta != 0) {
...
byte sound_prio_table[] = {0x14, // sound_0_fell_to_death
                           ...
                           0x00};
byte sound_pcspeaker_exists[] = {1, // sound_0_fell_to_death
                                 0, // sound_1_falling
...
void fix_sound_priorities() {
  ...
  // With PoP 1.3 sounds, the "guard hurt" sound didn't play when you hit a
  // guard directly after parrying.
  sound_prio_table[sound_10_sword_vs_sword] =
      0x0D; // moved below hit_user/hit_guard
}
...
void play_sound(int sound_id) {
  ...
  if (next_sound < 0 ||
      sound_prio_table[sound_id] <= sound_prio_table[next_sound]) {
    if (NULL == sound_pointers[sound_id])
      return;
    if (sound_pcspeaker_exists[sound_id] != 0 ||
        sound_pointers[sound_id]->type != sound_speaker) {
      next_sound = sound_id;
    }
  }
}
...
// seg000:136A
void load_chtab_from_file(int chtab_id, int resource, const char *filename,
                          int palette_bits) {
...
// seg009:12EF
void load_one_optgraf(chtab_type *chtab_ptr, dat_pal_type *pal_ptr, int base_id,
...
void load_more_opt_graf(const char *filename) {
  ...
  for (short graf_index = 0; graf_index < 8; ++graf_index) {
    /*if (...) */ {
      ...
      load_one_optgraf(chtab_addrs[id_chtab_6_environment], &area.palette, 1200,
                       optgraf_min[graf_index] - 1,
                       optgraf_max[graf_index] - 1);
    }
  }
  if (dathandle != NULL) {
    close_dat(dathandle);
...
// seg000:148D
int do_paused() {
  // Skip keyboard reading in RL mode — control_x/y/shift are set by the
  // RL action handler in play_level_2() and must not be overwritten.
  extern int rl_mode;
  if (rl_mode) return 0;
  ...
  if (is_ending_sequence && is_paused) {
    is_paused =
        0; // fix being able to pause the game during the ending sequence
  }
  if (is_paused) {
  ...
  // As we processed input for current gameplay tick change all input to reflect
...
void read_keyb_control() {
  ...
  if (key_states[SDL_SCANCODE_UP] & key_state ||
      key_states[SDL_SCANCODE_HOME] & key_state ||
      key_states[SDL_SCANCODE_PAGEUP] & key_state ||
      key_states[SDL_SCANCODE_KP_8] & key_state ||
      key_states[SDL_SCANCODE_KP_7] & key_state ||
      key_states[SDL_SCANCODE_KP_9] & key_state ||
      key_states[key_up] & key_state || key_states[key_jump_left] & key_state ||
      key_states[key_jump_right] & key_state) {
    control_y = CONTROL_HELD_UP;
  } else if (key_states[SDL_SCANCODE_CLEAR] & key_state ||
             key_states[SDL_SCANCODE_DOWN] & key_state ||
             key_states[SDL_SCANCODE_KP_5] & key_state ||
             key_states[SDL_SCANCODE_KP_2] & key_state ||
             key_states[key_down] & key_state) {
    control_y = CONTROL_HELD_DOWN;
  }
  if (key_states[SDL_SCANCODE_LEFT] & key_state ||
      key_states[SDL_SCANCODE_HOME] & key_state ||
      key_states[SDL_SCANCODE_KP_4] & key_state ||
      key_states[SDL_SCANCODE_KP_7] & key_state ||
      key_states[key_left] & key_state ||
      key_states[key_jump_left] & key_state) {
    control_x = CONTROL_HELD_LEFT;
  } else if (key_states[SDL_SCANCODE_RIGHT] & key_state ||
             key_states[SDL_SCANCODE_PAGEUP] & key_state ||
             key_states[SDL_SCANCODE_KP_6] & key_state ||
             key_states[SDL_SCANCODE_KP_9] & key_state ||
             key_states[key_right] & key_state ||
             key_states[key_jump_right] & key_state) {
    control_x = CONTROL_HELD_RIGHT;
  }

  if (key_states[SDL_SCANCODE_LSHIFT] & key_state ||
      key_states[SDL_SCANCODE_RSHIFT] & key_state ||
      key_states[key_action] & key_state)
    control_shift = CONTROL_HELD;
  else
    control_shift = CONTROL_RELEASED;
...
// We need a version of showmessage() which can detect modifier keys as well, in
// case someone wants to configure such a key for controls.
int showmessage_any_key(char *text, int arg_4, void *arg_0) {
  ...
  clear_kbd_buf();
  last_any_key_scancode = 0; // Don't leak the Enter keypress (which opened the
                             // dialog) into the dialog.
  do {
    idle();
...
void copy_screen_rect(const rect_type *source_rect_ptr) {
  ...
  if (upside_down) {
    ...
    /*target_rect_ptr->*/ target_rect.top =
        SCREEN_GAMEPLAY_HEIGHT - source_rect_ptr->bottom;
    /*target_rect_ptr->*/ target_rect.bottom =
        SCREEN_GAMEPLAY_HEIGHT - source_rect_ptr->top;
  } else {
    target_rect_ptr = source_rect_ptr;
  }
  method_1_blit_rect(onscreen_surface_, offscreen_surface, target_rect_ptr,
                     target_rect_ptr, 0);
#ifdef USE_LIGHTING
...
// seg000:17E6
void show_title() {
  load_opt_sounds(sound_50_story_2_princess,
                  sound_55_story_1_absence); // main theme, story, princess door
  dont_reset_time = 0;
  ...
  method_1_blit_rect(onscreen_surface_, offscreen_surface, &screen_rect,
                     &screen_rect, blitters_0_no_transp);
  ...
  method_1_blit_rect(onscreen_surface_, offscreen_surface, &rect_titles,
                     &rect_titles, blitters_0_no_transp);
  draw_full_image(TITLE_MAIN);
  do_wait(timer_0);
  ...
  //&screen_rect, blitters_0_no_transp);
  play_sound_from_buffer(
      sound_pointers[sound_55_story_1_absence]); // story 1: In the absence
  transition_ltr();
...
void transition_ltr() {
  ...
  // IIe. See: https://www.youtube.com/watch?v=7m7j2VuWhQ0
  ...
  for (short position = 0; position < 320; position += 2) {
    ...
    if (overshoot > 0 && overshoot < 10) {
      --overshoot;
      continue; // On slow systems (e.g. Raspberry Pi), allow the animation to
                // catch up, before refreshing the screen.
    }
    idle(); // modified
    do_paused();
    // Add an appropriate delay until the next frame, so that the animation
    // isn't instantaneous on fast CPUs.
    for (;;) {
      Uint64 current_counter = SDL_GetPerformanceCounter();
      int frametimes_elapsed =
          (int)((current_counter / counters_per_frame) -
                (last_transition_counter / counters_per_frame));
...
void save_game() {
  ...
  const char *save_path =
      get_save_path(custom_save_path, sizeof(custom_save_path));
...
void clear_screen_and_sounds() {
  ...
  for (index = 44; index < 57; ++index) {
          //continue; // don't release sounds? modern machines have enough
  memory free_sound(sound_pointers[index]); // added sound_pointers[index] =
  NULL;
  }
  */
  current_level = -1;
...
void load_all_sounds() {
  if (!use_custom_levelset || always_use_original_music) {
    ...
    // Put it here instead to use data/ for all music and sounds, not just for
    // those in data/music/.
...
void load_title_images(int bgcolor) {
  ...
  if (graphics_mode == gmMcgaVga) {
    ...
    if (NULL != chtab_title40) {
      SDL_SetPaletteColors(chtab_title40->images[0]->format->palette, &color,
                           14, 1);
    }
...
#ifdef USE_COPYPROT
// data:017A
const word copyprot_word[] = {9,  1, 6, 4, 5, 3, 6, 3,  4,  4, 3, 2, 12, 5,
                              13, 1, 9, 2, 2, 4, 9, 4,  11, 8, 5, 4, 1,  6,
                              2,  4, 6, 8, 4, 2, 7, 11, 5,  4, 1, 2};
// data:012A
const word copyprot_line[] = {2, 1, 5, 4, 3, 5, 1, 3, 7, 2,  2, 4, 6, 6,
                              2, 6, 3, 1, 2, 3, 2, 2, 3, 10, 5, 6, 5, 6,
                              3, 5, 7, 2, 2, 4, 5, 7, 2, 6,  5, 5};
// data:00DA
const word copyprot_page[] = {5, 3, 7, 3, 3, 4, 1, 5, 12, 5,  11, 10, 1,  2,
                              8, 8, 2, 4, 6, 1, 4, 7, 3,  2,  1,  7,  10, 1,
                              4, 3, 4, 1, 4, 1, 8, 1, 1,  10, 3,  3};
#endif
...
void show_copyprot(int where) {
  ...
  if (where) {
    ...
    is_show_time = 0;
    snprintf(sprintf_temp, sizeof(sprintf_temp), "WORD %d LINE %d PAGE %d",
             copyprot_word[copyprot_idx], copyprot_line[copyprot_idx],
             copyprot_page[copyprot_idx]);
...
void show_quotes() {
  ...
  if (demo_mode && need_quotes) {
    draw_rect(&screen_rect, color_0_black);
    show_text(&screen_rect, halign_left, valign_middle,
              tbl_quotes[which_quote]);
...
const char *splash_text_2 = "In-game, Esc opens a settings/quicksave menu.\n"
                            "\n"
#ifdef USE_REPLAY
...
void show_splash() {
  ...
  current_target_surface = onscreen_surface_;
  draw_rect(&screen_rect, color_0_black);
  show_text_with_color(&splash_text_1_rect, halign_center, valign_middle,
                       splash_text_1, color_15_brightwhite);
  show_text_with_color(&splash_text_2_rect, halign_center, valign_top,
                       splash_text_2, color_7_lightgray);

#ifdef USE_TEXT // Don't wait for a keypress if there is no text for the user to
                // read.
  int key = 0;
  do {
    idle();
    ...
    if (joy_input) {
      ...
      key_states[SDL_SCANCODE_LSHIFT] |=
          KEYSTATE_HELD; // close the splash screen using the gamepad
    }

    delay_ticks(1);

  } while (key == 0 && !(key_states[SDL_SCANCODE_LSHIFT] & KEYSTATE_HELD ||
                         key_states[SDL_SCANCODE_RSHIFT] & KEYSTATE_HELD));

  if ((key & WITH_CTRL) || (enable_quicksave && key == SDL_SCANCODE_F9) ||
      (enable_replay && key == SDL_SCANCODE_TAB)) {
    extern int last_key_scancode; // defined in seg009.c
    last_key_scancode =
        key; // can immediately do Ctrl+L, etc from the splash screen
  }
  key_states[SDL_SCANCODE_LSHIFT] &=
      ~KEYSTATE_HELD; // don't immediately start the game if Shift was pressed!
  key_states[SDL_SCANCODE_RSHIFT] &= ~KEYSTATE_HELD;
#endif
}

const char *get_writable_file_path(char *custom_path_buffer, size_t max_len,
                                   const char *file_name) {
  // If the SDLPOP_SAVE_PATH environment variable is set, put all saves into the
  // directory it points to. Otherwise, save to the home directory
  ...
  if (save_path != NULL && save_path[0] != '\0') {
    ...
    if (use_custom_levelset) {
      ...
      snprintf_check(custom_path_buffer, max_len, "%s/%s", save_path,
                     levelset_name);
#if defined WIN32 || _WIN32 || WIN64 || _WIN64
...
#endif
      snprintf_check(custom_path_buffer, max_len, "%s/%s/%s", save_path,
                     levelset_name, file_name);
    } else {
      snprintf_check(custom_path_buffer, max_len, "%s/%s", save_path,
                     file_name);
    }
  ...
  snprintf_check(custom_path_buffer, max_len, "%s/%s", mod_data_path,
                 file_name);
  return custom_path_buffer;
}
