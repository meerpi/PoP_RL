...
// Optional RL capture (state + frames). Enabled only when SDLPoP_RL_CAPTURE is set.
#include <stdlib.h>
...
// ---------------- RL capture (optional, controlled via environment variables) ----------------
// Enable by setting: SDLPoP_RL_CAPTURE=1
// Optional:
//   SDLPoP_RL_STATE_PATH=/path/to/state.jsonl   (default: rl_state.jsonl)
//   SDLPoP_RL_FRAMES_PATH=/path/to/frames.rgb  (default: rl_frames.rgb)
// Notes:
// - Frames are written as raw RGB24, one frame after another, size = (w*h*3) bytes.
// - State is written as JSONL, one line per captured frame.
static int rl_capture_inited = 0;
static int rl_capture_enabled = 0;
static FILE* rl_state_fp = NULL;
static FILE* rl_frames_fp = NULL;
static unsigned int rl_frame_counter = 0;  // always-incrementing frame counter for RL

static void rl_capture_close(void) {
	if (rl_state_fp) { fflush(rl_state_fp); fclose(rl_state_fp); rl_state_fp = NULL; }
	if (rl_frames_fp) { fflush(rl_frames_fp); fclose(rl_frames_fp); rl_frames_fp = NULL; }
}

static void rl_capture_init(void) {
	if (rl_capture_inited) return;
	rl_capture_inited = 1;

	const char* enable = getenv("SDLPoP_RL_CAPTURE");
	rl_capture_enabled = (enable && enable[0] != '\0' && enable[0] != '0');
	if (!rl_capture_enabled) return;

	const char* state_path = getenv("SDLPoP_RL_STATE_PATH");
	const char* frames_path = getenv("SDLPoP_RL_FRAMES_PATH");
	if (!state_path || state_path[0] == '\0') state_path = "rl_state.jsonl";
	if (!frames_path || frames_path[0] == '\0') frames_path = "rl_frames.rgb";

	rl_state_fp = fopen(state_path, "wb");
	rl_frames_fp = fopen(frames_path, "wb");
	if (!rl_state_fp || !rl_frames_fp) {
		fprintf(stderr, "RL capture: failed to open output files (state=%s frames=%s)\n", state_path, frames_path);
		rl_capture_enabled = 0;
		rl_capture_close();
		return;
	}

	// Header line helps the reader validate format/dimensions.
	fprintf(rl_state_fp, "{\"type\":\"header\",\"w\":%d,\"h\":%d,\"format\":\"RGB24\"}\n", pop_window_width, pop_window_height);
	fflush(rl_state_fp);

	atexit(rl_capture_close);
}

static void rl_capture_frame_and_state(SDL_Surface* surface) {
	if (!rl_capture_inited) rl_capture_init();
	if (!rl_capture_enabled) return;
	if (!surface || !surface->pixels || surface->w <= 0 || surface->h <= 0) return;

	SDL_Surface* src = surface;
	SDL_Surface* tmp = NULL;
	if (surface->format->BytesPerPixel != 3) {
		tmp = SDL_ConvertSurfaceFormat(surface, SDL_PIXELFORMAT_RGB24, 0);
		if (!tmp) return;
		src = tmp;
	}
	...
	const byte* p = (const byte*)src->pixels;

	for (int y = 0; y < h; y++) {
		fwrite(p + y * pitch, 1, (size_t)row_bytes, rl_frames_fp);
	}

	fprintf(rl_state_fp,
		"{\"type\":\"step\",\"frame_id\":%u,\"tick\":%u,\"level\":%d,\"room\":%d,\"kid\":{\"x\":%d,\"y\":%d,\"col\":%d,\"row\":%d,\"room\":%d,\"action\":%d,\"alive\":%d},\"hp\":{\"cur\":%d,\"max\":%d},\"time\":{\"rem_min\":%d,\"rem_tick\":%d},\"frame\":{\"w\":%d,\"h\":%d}}\n",
		rl_frame_counter++,
		(unsigned int)curr_tick,
		(int)current_level,
		(int)drawn_room,
		(int)Kid.x, (int)Kid.y, (int)Kid.curr_col, (int)Kid.curr_row, (int)Kid.room, (int)Kid.action, (int)Kid.alive,
		(int)hitp_curr, (int)hitp_max,
		(int)rem_min, (int)rem_tick,
		w, h);
...
void update_screen() {
	draw_overlay();
	SDL_Surface* surface = get_final_surface();
	rl_capture_frame_and_state(surface);
	init_scaling();
	if (scaling_type == 1) {
