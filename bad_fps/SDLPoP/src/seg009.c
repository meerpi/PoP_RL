...
void find_share_dir(void) {
  if (found_share_dir)
    return;
  snprintf_check(share_dir, POP_MAX_PATH - 1, "%s/%s", SHARE_PATH,
                 POP_DIR_NAME);
...
const char *find_first_file_match(char *dst, int size, char *format,
                                  const char *filename) {
  find_exe_dir();
#if defined WIN32 || _WIN32 || WIN64 || _WIN64
...
const char *locate_file_(const char *filename, char *path_buffer,
                         int buffer_size) {
  if (file_exists(filename)) {
    return filename;
  } else {
    // If failed, it may be that SDLPoP is being run from the wrong different
    // working directory. We can try to rescue the situation by loading from the
...
// This hack is needed because SDL uses UTF-8 everywhere (even in argv!), but
// fopen on Windows uses whatever code page is currently set.
...
#else
  // There is a _wstat() function on Dev-C++ as well, but it expects the second
  // argument to be a different type than stat().
...
// Directory listing using dirent.h is available using MinGW on Windows, but not
// using MSVC (need to use Win32 API).
// - Under GNU/Linux, etc (or if compiling with MinGW on Windows), we can use
// dirent.h
// - Under Windows, we'd like to directly call the Win32 API. (Note: MSVC does
// not include dirent.h) NOTE: If we are using MinGW, we'll opt to use the Win32
// API as well: dirent.h would just wrap Win32 anyway!
...
create_directory_listing_and_find_first_file(const char *directory,
                                             const char *extension) {
  directory_listing_type *directory_listing =
      calloc(1, sizeof(directory_listing_type));
  ...
  directory_listing->search_handle =
      FindFirstFileW(search_pattern_UTF16, &directory_listing->find_data);
...
char *
get_current_filename_from_directory_listing(directory_listing_type *data) {
...
const char *check_param(const char *param) {
  ...
  for (short arg_index = 1; arg_index < g_argc; ++arg_index) {
    ...
    // Filenames (e.g. replays) should never be a valid 'normal' param so we
    // should skip these to prevent conflicts. We can lazily distinguish
    // filenames from non-filenames by checking whether they have a dot in them.
    // (Assumption: all relevant files, e.g. replay files, have some file
    // extension anyway)
    if (strchr(curr_arg, '.') != NULL) {
      continue;
    }

    // List of params that expect a specifier ('sub-') arg directly after it
    // (e.g. the mod's name, after "mod" arg) Such sub-args may conflict with
    // the normal params (so, we should 'skip over' them)
    static const char params_with_one_subparam[][16] = {"mod",
                                                        "validate",
                                                        /*...*/};

    bool curr_arg_has_one_subparam = false;
    for (int i = 0; i < COUNT(params_with_one_subparam); ++i) {
      if (strncasecmp(curr_arg, params_with_one_subparam[i],
                      strlen(params_with_one_subparam[i])) == 0) {
    ...
    if (curr_arg_has_one_subparam) {
      ...
      // 1: skip over the next arg                (if we are NOT checking for
      // this specific param) 2: return a pointer below to the SUB-arg (if we
      // ARE checking for this specific param)
      ++arg_index;
      if (!(arg_index < g_argc))
...
static FILE *open_dat_from_root_or_data_dir(const char *filename) {
  ...
  if (fp == NULL) {
    ...
    if (!file_exists(data_path)) {
      find_first_file_match(data_path, sizeof(data_path), "%s/data/%s",
                            filename);
    }

    // verify that this is a regular file and not a directory (otherwise, don't
    // open)
    struct stat path_stat;
...
dat_type *open_dat(const char *filename, int optional) {
  ...
  if (!use_custom_levelset) {
    ...
    // Don't complain about missing data files if we are only looking in the mod
    // folder, because they might exist in the data folder. (This is possible
    // only if open_dat() was called by load_all_sounds().)
    if (!skip_mod_data_files && skip_normal_data_files)
      optional = 1;

    if (!skip_mod_data_files &&
        !(always_use_original_graphics && optional == 'G')) {
      char filename_mod[POP_MAX_PATH];
      // before checking the root directory, first try mods/MODNAME/
      snprintf_check(filename_mod, sizeof(filename_mod), "%s/%s", mod_data_path,
                     filename);
  ...
  if (fp != NULL) {
    ...
    // showmessage will crash if we call it before certain things are
    // initialized! Solution: In pop_main(), I moved the first open_dat() call
    // after init_copyprot_dialog().
    // /*
    // There is no DAT file, verify whether the corresponding directory exists.
    char filename_no_ext[POP_MAX_PATH];
    // strip the .DAT file extension from the filename (use folders simply named
    // TITLE, KID, VPALACE, etc.)
    strncpy(filename_no_ext, pointer->filename, sizeof(filename_no_ext));
    size_t len = strlen(filename_no_ext);
    if (len >= 5 && filename_no_ext[len - 4] == '.') {
      filename_no_ext[len - 4] =
          '\0'; // terminate, so ".DAT" is deleted from the filename
    }
    ...
    if (result != 0 || !S_ISDIR(path_stat.st_mode)) {
      ...
      snprintf_check(error_message, sizeof(error_message),
                     "Cannot find a required data file: %s or folder: "
                     "%s\nPress any key to quit.",
                     filename, foldername);
      if (onscreen_surface_ != NULL &&
          copyprot_dialog != NULL) { // otherwise showmessage will crash
...
void set_loaded_palette(dat_pal_type *palette_ptr) {
  ...
  for (dest_row = dest_index = source_row = 0; dest_row < 16;
       ++dest_row, dest_index += 0x10) {
...
// seg009:104E
chtab_type *load_sprites_from_file(int resource, int palette_bits,
                                   int quit_on_error) {
  // int has_palette_bits = 1;
  dat_shpl_type *shpl =
      (dat_shpl_type *)load_from_opendats_alloc(resource, "pal", NULL, NULL);
  if (shpl == NULL) {
    printf("Can't load sprites from resource %d.\n", resource);
    if (quit_on_error) {
      char error_message[256];
      // Unfortunately we don't know at this point which data file is missing.
      // So we use the name of the last opened DAT file. It's also possible that
      // the DAT file exists and it just doesn't contain the needed resource.
      snprintf_check(error_message, sizeof(error_message),
                     "Can't load sprites from resource %d.\nThe last opened "
                     "data file is: %s\nPress any key to quit.",
                     resource, dat_chain_ptr->filename);
      showmessage(error_message, 1, &key_test_quit);
      quit(1);
    }
    return NULL;
    ...
    if (image != NULL) {
      /*
                              if (SDL_SetSurfaceAlphaMod(image, 0) != 0) {
                                      sdlperror("load_sprites_from_file:
         SDL_SetAlpha"); quit(1);
                              }
      */
      /*
...
// seg009:8D1C
void decompress_rle_ud(byte *destination, const byte *source, int dest_length,
...
// seg009:91AD
byte *decompress_lzg_ud(byte *dest, const byte *source, int dest_length,
...
void decompr_img(byte *dest, const image_data_type *source, int decomp_size,
  ...
  case 2: // RLE up-to-down
    decompress_rle_ud(dest, source->data, decomp_size, stride,
                      SDL_SwapLE16(source->height));
    break;
  case 3: // LZG left-to-right
    decompress_lzg_lr(dest, source->data, decomp_size);
    break;
  case 4: // LZG up-to-down
    decompress_lzg_ud(dest, source->data, decomp_size, stride,
                      SDL_SwapLE16(source->height));
    break;
  }
}
...
byte *conv_to_8bpp(byte *in_data, int width, int height, int stride,
                   int depth) {
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (int x_pixel = 0, x_byte = 0; x_byte < stride; ++x_byte) {
      ...
      for (int pixel_in_byte = 0;
           pixel_in_byte < pixels_per_byte && x_pixel < width;
           ++pixel_in_byte, ++x_pixel) {
        shift -= depth;
...
image_type *decode_image(image_data_type *image_data, dat_pal_type *palette) {
  ...
  for (int y = 0; y < height; ++y) {
    ...
    memcpy((byte *)image->pixels + y * image->pitch, image_8bpp + y * width,
           width);
  }
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    colors[i].a =
        SDL_ALPHA_OPAQUE; // SDL2's SDL_Color has a fourth alpha component
  }
  // Force 0th color to be black for non-transparent blitters. (hitpoints,
  // shadow) This is needed to remove the colored rectangles around hitpoints
  // and the shadow, when using Brain's SNES graphics for example.
  colors[0].r = 0;
  colors[0].g = 0;
  colors[0].b = 0;
  colors[0].a = SDL_ALPHA_TRANSPARENT;
  SDL_SetPaletteColors(image->format->palette, colors, 0,
...
image_type *load_image(int resource_id, dat_pal_type *palette) {
  ...
  int size;
  void *image_data =
      load_from_opendats_alloc(resource_id, "png", &result, &size);
  ...
  if (image != NULL) {
    // should immediately start using the onscreen pixel format, so conversion
    // will not be needed
    ...
    /*
                    if (SDL_SetSurfaceAlphaMod(image, 0) != 0) { //sdl 1.2:
       SDL_SetAlpha removed sdlperror("load_image: SDL_SetAlpha"); quit(1);
                    }
    */
    //		image_type* colored_image = SDL_ConvertSurfaceFormat(image,
    // SDL_PIXELFORMAT_ARGB8888, 0); 		if (!colored_image) {
...
// seg009:13C4
void draw_image_transp(image_type *image, image_type *mask, int xpos,
                       int ypos) {
  if (graphics_mode == gmMcgaVga) {
...
int set_joy_mode() {
  ...
  if (SDL_NumJoysticks() < 1) {
    ...
    // We have a joystick connected, but it's NOT compatible with the
    // SDL_GameController interface, so we resort to the classic SDL_Joystick
    // interface instead
    else {
...
surface_type *make_offscreen_buffer(const rect_type *rect) {
  ...
  return SDL_CreateRGBSurface(0, rect->right, rect->bottom, 24, Rmsk, Gmsk,
                              Bmsk, 0);
#else
  return SDL_CreateRGBSurface(0, rect->right, rect->bottom, 32, Rmsk, Gmsk,
                              Bmsk, Amsk);
#endif
  // return surface;
}
...
// seg009:17BD
void free_surface(surface_type *surface) { SDL_FreeSurface(surface); }
...
void fade_in_2(surface_type *source_surface, int which_rows) {
  // stub
  method_1_blit_rect(onscreen_surface_, source_surface, &screen_rect,
                     &screen_rect, 0);
}
...
/*const*/ byte hc_font_data[] = {
    };
...
static void load_font_character_offsets(rawfont_type *data) {
  ...
  for (int index = 0; index < n_chars; ++index) {
    ...
    int image_bytes =
        SDL_SwapLE16(image_data->height) * calc_stride(image_data);
...
font_type load_font_from_data(/*const*/ rawfont_type *data) {
  ...
  // Allow loading a font even if the offsets for each character image were not
  // supplied in the raw data.
  if (SDL_SwapLE16(data->offsets[0]) == 0) {
    load_font_character_offsets(data);
  }
  chtab_type *chtab =
      malloc(sizeof(chtab_type) + sizeof(image_type *) * n_chars);
  ...
  for (int index = 0, chr = data->first_char; chr <= data->last_char;
       ++index, ++chr) {
    /*const*/ image_data_type *image_data =
        (/*const*/ image_data_type *)((/*const*/ byte *)data +
                                      SDL_SwapLE16(data->offsets[index]));
    // image_data->flags=0;
    if (image_data->height == SDL_SwapLE16(0))
      image_data->height =
          SDL_SwapLE16(1); // HACK: decode_image() returns NULL if height==0.
    image_type *image;
...
int get_char_width(byte character) {
  ...
  if (character <= font->last_char && character >= font->first_char) {
    ...
    if (image != NULL) {
      ...
      if (width)
        width += font->space_between_chars;
    }
  }
  return width;
}
...
int draw_text_character(byte character) {
  ...
  if (character <= font->last_char && character >= font->first_char) {
    image_type *image =
        font->chtab
            ->images[character - font->first_char]; // char_ptrs[character -
                                                    // font->first_char];
    if (image != NULL) {
      method_3_blit_mono(image, textstate.current_x,
                         textstate.current_y - font->height_above_baseline,
                         textstate.textblit, textstate.textcolor);
...
// seg009:3F01
const rect_type *draw_text(const rect_type *rect_ptr, int x_align, int y_align,
...
#define MAX_LINES 100
  ...
  do {
    int line_length =
        find_linebreak(line_start, rem_length, rect_width, x_align);
    if (line_length == 0)
      break;
  ...
  font_type *font = textstate.ptr_font;
  font_line_distance = font->height_above_baseline +
                       font->height_below_baseline + font->space_between_lines;
  ...
  for (int i = 0; i < num_lines; ++i) {
    ...
    if (x_align < 0 && *line_pos == ' ' && i != 0 && *(line_pos - 1) != '\n') {
      // Skip over space if it's not at the beginning of a line.
      ++line_pos;
      --line_length;
      if (line_length != 0 && *line_pos == ' ' && *(line_pos - 2) == '.') {
...
// seg009:3E4F
void show_text(const rect_type *rect_ptr, int x_align, int y_align,
               const char *text) {
  // stub
  // printf("show_text: %s\n",text);
...
// seg009:04FF
void show_text_with_color(const rect_type *rect_ptr, int x_align, int y_align,
...
void init_copyprot_dialog() {
  copyprot_dialog =
      make_dialog_info(&dialog_settings, &dialog_rect_1, &dialog_rect_1, NULL);
...
int showmessage(char *text, int arg_4, void *arg_0) {
  ...
  //  In the disassembly there is some messing with the current_target_surface
  //  and font (?) However, this does not seem to be strictly necessary
  if (NULL == offscreen_surface)
    offscreen_surface =
        make_offscreen_buffer(&screen_rect); // In case we get an error before
                                             // there is an offsceen buffer
  method_1_blit_rect(offscreen_surface, onscreen_surface_,
                     &copyprot_dialog->peel_rect, &copyprot_dialog->peel_rect,
                     0);
  ...
  show_text_with_color(&rect, halign_center, valign_middle, text,
  ...
  need_full_redraw = 1; // lazy: instead of neatly restoring only the relevant
                        // part, just redraw the whole screen
  return key;
}

// seg009:08FB
dialog_type *make_dialog_info(dialog_settings_type *settings,
                              rect_type *dialog_rect, rect_type *text_rect,
                              peel_type *dialog_peel) {
  ...
  if (text_rect !=
      NULL) { // does not seem to be quite right; see seg009:0948 (?)
...
void calc_dialog_peel_rect(dialog_type *dialog) {
  ...
  dialog->peel_rect.right =
      dialog->text_rect.right + settings->right_border + settings->shadow_right;
  dialog->peel_rect.bottom = dialog->text_rect.bottom +
                             settings->bottom_border + settings->shadow_bottom;
}
...
void dialog_method_2_frame(dialog_type *dialog) {
  ...
  // Draw outer border
  rect = (rect_type){peel_top, peel_left, peel_bottom - shadow_bottom,
                     peel_right - shadow_right};
  draw_rect(&rect, color_0_black);
  // Draw shadow (right)
  rect =
      (rect_type){text_top, peel_right - shadow_right, peel_bottom, peel_right};
  draw_rect(&rect, get_text_color(0, color_8_darkgray /*dialog's shadow*/, 0));
  // Draw shadow (bottom)
  rect = (rect_type){peel_bottom - shadow_bottom, text_left, peel_bottom,
                     peel_right};
  draw_rect(&rect, get_text_color(0, color_8_darkgray /*dialog's shadow*/, 0));
  // Draw inner border (left)
  rect = (rect_type){peel_top + outer_border, peel_left + outer_border,
                     text_bottom, text_left};
  draw_rect(&rect, color_15_brightwhite);
  // Draw inner border (top)
  rect =
      (rect_type){peel_top + outer_border, text_left, text_top,
                  text_right + dialog->settings->right_border - outer_border};
  draw_rect(&rect, color_15_brightwhite);
  // Draw inner border (right)
  rect.top = text_top;
  rect.left = text_right;
  rect.bottom =
      text_bottom + bottom_border - outer_border; // (rect.right stays the same)
  draw_rect(&rect, color_15_brightwhite);
  // Draw inner border (bottom)
  rect = (rect_type){text_bottom, peel_left + outer_border,
                     text_bottom + bottom_border - outer_border, text_right};
  draw_rect(&rect, color_15_brightwhite);
}
...
int get_text_center_y(const rect_type *rect) {
  ...
  empty_height = rect->bottom - font->height_above_baseline -
                 font->height_below_baseline - rect->top;
  return ((empty_height - empty_height % 2) >> 1) +
         font->height_above_baseline + empty_height % 2 + rect->top;
}
...
// seg009:053C
int input_str(const rect_type *rect, char *buffer, int max_length,
              const char *initial, int has_initial, int arg_4, int color,
              int bgcolor) {
  ...
  do {
    ...
    do {
      ...
      while (!has_timer_stopped(timer_0) && (key = key_test_quit()) == 0)
        idle();
    } while (1);
    ...
    if (length != 0 && (key == SDL_SCANCODE_BACKSPACE ||
      ...
      // restore_curr_pos?();
      draw_text_cursor(current_xpos, ypos, color);
    } else if (entered_char >= 0x20 && entered_char <= 0x7E &&
               length < max_length) {
      // Would the new character make the cursor go past the right side of the
      // rect?
      if (get_char_width('_') + get_char_width(entered_char) + current_xpos <
          rect->right) {
        draw_text_cursor(current_xpos, ypos, bgcolor);
        set_curr_pos(current_xpos, ypos);
        /*current_target_surface->*/ textstate.textcolor = color;
...
// seg009:39CE
rect_type *shrink2_rect(rect_type *target_rect, const rect_type *source_rect,
...
void restore_peel(peel_type *peel_ptr) {
  ...
  // peel_ptr.rect.top); // debug
  method_6_blit_img_to_scr(peel_ptr->peel, peel_ptr->rect.left,
                           peel_ptr->rect.top, /*0x10*/ 0);
  free_peel(peel_ptr);
  // SDL_FreeSurface(peel_ptr.peel);
}
...
peel_type *read_peel_from_screen(const rect_type *rect) {
  ...
  SDL_Surface *peel_surface =
      SDL_CreateRGBSurface(0, rect->right - rect->left,
                           rect->bottom - rect->top, 24, Rmsk, Gmsk, Bmsk, 0);
#else
  ...
  rect_type target_rect = {0, 0, rect->right - rect->left,
                           rect->bottom - rect->top};
  method_1_blit_rect(result->peel, current_target_surface, &target_rect, rect,
                     0);
  return result;
}

// seg009:3D95
int intersect_rect(rect_type *output, const rect_type *input1,
                   const rect_type *input2) {
...
// seg009:4063
rect_type *union_rect(rect_type *output, const rect_type *input1,
                      const rect_type *input2) {
...
// Tracks how long the last (partially played) speaker note has been playing
...
// Decoder for the currently playing OGG sound. (This also holds the playback
// position.)
...
short square_wave_state = 4000; // If the amplitude is too high, the speaker
...
void generate_square_wave(byte *stream, float note_freq, int samples) {
  ...
  while (samples_left > 0) {
    if (square_wave_samples_since_last_flip > half_period_in_samples) {
      ...
      // Note(Falcury): not completely sure that this is the right way to
      // prevent glitches in the sound... Because I can still hear some hiccups,
      // e.g. in the music. Especially when switching between notes.
      square_wave_samples_since_last_flip -= half_period_in_samples;
    } else {
      int samples_until_next_flip =
          (int)(half_period_in_samples - square_wave_samples_since_last_flip);
...
void speaker_callback(void *userdata, Uint8 *stream, int len) {
  ...
  while (total_samples_left > 0) {
    ...
    int note_samples_to_emit = MIN(
        note_length_in_samples - current_speaker_note_samples_already_emitted,
        total_samples_left);
    ...
    if (SDL_SwapLE16(note->frequency) <= 0x01 /*rest*/) {
      ...
      generate_square_wave(stream, (float)SDL_SwapLE16(note->frequency),
                           note_samples_to_emit);
    }
    stream += copy_len;
...
void digi_callback(void *userdata, Uint8 *stream, int len) {
  ...
  if (is_sound_on) {
    ...
    // In case the sound does not fill the buffer: fill the rest of the buffer
    // with silence.
...
void ogg_callback(void *userdata, Uint8 *stream, int len) {
  ...
  int samples_filled;
  if (is_sound_on) {
    samples_filled = stb_vorbis_get_samples_short_interleaved(
        ogg_decoder, output_channels, (short *)stream, len / sizeof(short));
    if (samples_filled < samples_requested) {
      // In case the sound does not fill the buffer: fill the rest of the buffer
      // with silence.
      int bytes_filled = samples_filled * bytes_per_sample;
      int remaining_bytes =
          (samples_requested - samples_filled) * bytes_per_sample;
      memset(stream + bytes_filled, digi_audiospec->silence, remaining_bytes);
    }
  } else {
    // If sound is off: Mute the sound, but keep track of where we are.
    memset(stream, digi_audiospec->silence, len);
    // Let the decoder run normally (to advance the position), but discard the
    // result.
    byte *discarded_samples = alloca(len);
    samples_filled = stb_vorbis_get_samples_short_interleaved(
        ogg_decoder, output_channels, (short *)discarded_samples,
        len / sizeof(short));
  }
...
void audio_callback(void *userdata, Uint8 *stream_orig, int len_orig) {
  ...
  // Note: music sounds and digi sounds are allowed to play simultaneously (will
  // be blended together) I.e., digi sounds and music will not cut each other
  // short.
  if (midi_playing) {
  ...
  if (audio_speed > 1) {
    ...
    if (!cvt_initialized) {
      SDL_BuildAudioCVT(&cvt, digi_audiospec->format, digi_audiospec->channels,
                        digi_audiospec->freq * audio_speed,
                        digi_audiospec->format, digi_audiospec->channels,
                        digi_audiospec->freq);
...
void init_digi() {
  ...
  if (version.major <= 2 && version.minor <= 0 && version.patch <= 3) {
    // In versions before 2.0.4, 16-bit audio samples don't work properly (the
    // sound becomes garbled). See:
    // https://bugzilla.libsdl.org/show_bug.cgi?id=2389 Workaround: set the
    // audio format to 8-bit, if we are linking against an older SDL2 version.
    desired_audioformat = AUDIO_U8;
    printf("Your SDL.dll is older than 2.0.4. Using 8-bit audio format to work "
           "around resampling bug.");
  } else {
...
sound_buffer_type *load_sound(int index) {
  ...
  // printf("load_sound(%d)\n", index);
  init_digi();
  if (enable_music && !digi_unavailable && result == NULL && index >= 0 &&
      index < max_sound_id) {
    ...
    if (sound_names != NULL && sound_name(index) != NULL) {
      ...
      do {
        ...
        if (!skip_mod_data_files) {
          // before checking the root directory, first try mods/MODNAME/
          snprintf_check(filename, sizeof(filename), "%s/music/%s.ogg",
                         mod_data_path, sound_name(index));
          fp = fopen(filename, "rb");
        }
        if (fp == NULL && !skip_normal_data_files) {
          snprintf_check(filename, sizeof(filename), "data/music/%s.ogg",
                         sound_name(index));
        ...
        // Decoding the entire file immediately would make the loading time much
        // longer. However, we can also create the decoder now, and only use it
        // when we are actually playing the file. (In the audio callback, we'll
        // decode chunks of samples to the output stream, as needed).
        int error = 0;
        stb_vorbis *decoder =
            stb_vorbis_open_memory(file_contents, (int)file_size, &error, NULL);
        if (decoder == NULL) {
        ...
        result->ogg.total_length =
            stb_vorbis_stream_length_in_samples(decoder) * sizeof(short);
        result->ogg.file_contents =
            file_contents; // Remember in case we want to free the sound later.
  ...
  if (result == NULL) {
    ...
    result = (sound_buffer_type *)load_from_opendats_alloc(index + 10000, "bin",
                                                           NULL, NULL);
  }
...
void play_ogg_sound(sound_buffer_type *buffer) {
  ...
  // Need to rewind the music, or else the decoder might continue where it left
  // off, the last time this sound played.
...
bool determine_wave_version(sound_buffer_type *buffer,
                            waveinfo_type *waveinfo) {
  int version = wave_version;
  if (version == -1) {
    // Determine the version of the wave data.
    version = 0;
    if (buffer->digi.sample_size == 8)
      version += 1;
    if (buffer->digi_new.sample_size == 8)
      version += 2;
...
sound_buffer_type *convert_digi_sound(sound_buffer_type *digi_buffer) {
  ...
  if (digi_unavailable)
    return NULL;
  waveinfo_type waveinfo;
  if (false == determine_wave_version(digi_buffer, &waveinfo))
    return NULL;
  ...
  int expanded_frames =
      source_length * digi_audiospec->freq / waveinfo.sample_rate;
  int expanded_length = expanded_frames * 2 * sizeof(short);
  sound_buffer_type *converted_buffer =
      malloc(sizeof(sound_buffer_type) + expanded_length);
  ...
  for (int i = 0; i < expanded_frames; ++i) {
    ...
    if (src_frame_0 >= waveinfo.sample_count - 1) {
      ...
      interpolated_sample =
          (short)((1.0f - alpha) * sample_0 + alpha * sample_1);
    }
...
void apply_aspect_ratio() {
  // Allow us to use a consistent set of screen co-ordinates, even if the screen
  // size changes
...
void window_resized() {
  ...
  if (use_integer_scaling) {
    ...
    // Because then the only suitable integer scaling factor is zero, i.e. the
    // picture disappears.
...
void init_overlay(void) {
  ...
  if (!initialized) {
    overlay_surface =
        SDL_CreateRGBSurface(0, 320, 200, 32, Rmsk, Gmsk, Bmsk, Amsk);
...
void init_scaling(void) {
  ...
  if (texture_sharp == NULL) {
    texture_sharp = SDL_CreateTexture(renderer_, SDL_PIXELFORMAT_RGB24,
  ...
  if (scaling_type == 1) {
...
#ifdef __amigaos4__
      overlay_surface =
          SDL_CreateRGBSurface(0, 320 * 2, 200 * 2, 24, Rmsk, Gmsk, Bmsk, 0);
#else
      onscreen_surface_2x =
          SDL_CreateRGBSurface(0, 320 * 2, 200 * 2, 24, Rmsk, Gmsk, Bmsk, 0);
#endif
    ...
    if (texture_fuzzy == NULL) {
      ...
      int access = is_renderer_targettexture_supported
                       ? SDL_TEXTUREACCESS_TARGET
                       : SDL_TEXTUREACCESS_STREAMING;
      texture_fuzzy = SDL_CreateTexture(renderer_, SDL_PIXELFORMAT_RGB24,
                                        access, 320 * 2, 200 * 2);
    ...
    if (texture_blurry == NULL) {
      SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "1");
      texture_blurry = SDL_CreateTexture(renderer_, SDL_PIXELFORMAT_RGB24,
                                         SDL_TEXTUREACCESS_STREAMING, 320, 200);
      SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "0");
    }
...
void set_gr_mode(byte grmode) {
  ...
  if (enable_controller_rumble) {
    if (SDL_InitSubSystem(SDL_INIT_HAPTIC) != 0) {
      printf("Warning: Haptic subsystem unavailable, ignoring "
  ...
  Uint32 flags = 0;
  if (!start_fullscreen)
    start_fullscreen = check_param("full") != NULL;
  if (start_fullscreen)
  ...
  if (use_correct_aspect_ratio && pop_window_width == 640 &&
      pop_window_height == 400) {
    pop_window_height = 480;
  }

#if _WIN32
  // Tell Windows that the application is DPI aware, to prevent unwanted bitmap
  // stretching. SetProcessDPIAware() is only available on Windows Vista and
  // later, so we need to load it dynamically.
  typedef BOOL(WINAPI * dpiaware)(void);
  HMODULE user32dll = LoadLibraryA("User32.dll");
  if (user32dll) {
    dpiaware SetProcessDPIAware =
        (dpiaware)GetProcAddress(user32dll, "SetProcessDPIAware");
...
#endif
    window_ = SDL_CreateWindow(WINDOW_TITLE, SDL_WINDOWPOS_UNDEFINED,
                               SDL_WINDOWPOS_UNDEFINED, pop_window_width,
                               pop_window_height, flags);
  ...
  renderer_ =
      SDL_CreateRenderer(window_, -1, flags | SDL_RENDERER_TARGETTEXTURE);
...
#else
    printf("Warning: You need to compile with SDL 2.0.5 or newer for the "
           "use_integer_scaling option.\n");
#endif
  }
  ...
  /* Migration to SDL2: everything is still blitted to onscreen_surface_,
   * however: SDL2 renders textures to the screen instead of surfaces; so, every
   * screen update causes the onscreen_surface_ to be copied into the
   * target_texture, which is subsequently displayed. The function handling the
   * screen updates is update_screen()
   * */
  onscreen_surface_ =
      SDL_CreateRGBSurface(0, 320, 200, 24, Rmsk, Gmsk, Bmsk, 0);
  if (onscreen_surface_ == NULL) {
  ...
  // SDL_DEFAULT_REPEAT_INTERVAL) != 0) {  //deprecated
  // sdlperror("set_gr_mode: SDL_EnableKeyRepeat"); 		quit(1);
  //	}
...
void draw_overlay(void) {
  ...
  else if (fixes->fix_quicksave_during_feather && is_feather_timer_displayed &&
           start_level > 0 && is_feather_fall > 0) {
    overlay = 3; // Feather timer overlay
  }
#endif
#ifdef USE_MENU
  // Menu overlay - not drawn here directly, only copied from the overlay
  // surface.
  ...
  if (overlay != 0) {
    ...
    if (overlay == 1) {
      ...
      if (rem_min < 0) {
        ...
        snprintf(timer_text, sizeof(timer_text), "%02d:%02d:%02d", rem_min - 1,
                 rem_tick / 12, rem_tick % 12);
      }
      int expected_numeric_chars = 6;
      int extra_numeric_chars =
          MAX(0, (int)strnlen(timer_text, sizeof(timer_text)) - 8);
...
#ifdef USE_REPLAY
      // During playback, display the number of ticks since start, if the timer
      // is shown (debug cheats: T).
      if (replaying) {
        char ticks_text[12];
      ...
      drawn_rect =
          timer_box_rect; // Only need to blit this bit to the merged_surface.
#endif
      ...
      rect_type timer_text_rect = {2, 2, 10, 100};
      draw_rect_with_alpha(&timer_box_rect, color_0_black, 128);
      show_text_with_color(&timer_text_rect, halign_left, valign_top,
                           timer_text, color_10_brightgreen);

      drawn_rect =
          timer_box_rect; // Only need to blit this bit to the merged_surface.
#endif
    } else {
      drawn_rect = screen_rect; // We'll blit the whole contents of
                                // overlay_surface to the merged_surface.
    }
    SDL_Rect sdl_rect;
...
void update_screen() {
  ...
  if (scaling_type == 1) {
    ...
    // First scale to double size with nearest-neighbor scaling, then scale to
    // full screen with smooth scaling. The result is not as blurry as if we did
    // only a smooth scaling, but not as sharp as if we did only
...
void load_from_opendats_metadata(int resource_id, const char *extension,
                                 FILE **out_fp, data_location *result,
                                 byte *checksum, int *size,
                                 dat_type **out_pointer) {
  char image_filename[POP_MAX_PATH];
  FILE *fp = NULL;
  *result = data_none;
  // Go through all open DAT files.
  for (dat_type *pointer = dat_chain_ptr; fp == NULL && pointer != NULL;
    ...
    *out_pointer = pointer;
      ...
      if (i < SDL_SwapLE16(dat_table->res_count)) {
        ...
        *size = SDL_SwapLE16(dat_table->entries[i].size);
        if (strcmp(extension, "png") == 0 && *size <= 2) {
          ...
          *result = data_none;
          *size = 0;
        } else if (fseek(fp, SDL_SwapLE32(dat_table->entries[i].offset),
                         SEEK_SET) ||
    ...
    if (*result == data_none) {
      // If it's a directory:
      char filename_no_ext[POP_MAX_PATH];
      // strip the .DAT file extension from the filename (use folders simply
      // named TITLE, KID, VPALACE, etc.)
      strncpy(filename_no_ext, pointer->filename, sizeof(filename_no_ext));
      size_t len = strlen(filename_no_ext);
      if (len >= 5 && filename_no_ext[len - 4] == '.') {
        filename_no_ext[len - 4] =
      ...
      snprintf_check(image_filename, sizeof(image_filename), "data/%s/res%d.%s",
      ...
      if (!use_custom_levelset) {
        ...
        if (!skip_mod_data_files) {
          ...
          snprintf_check(image_filename_mod, sizeof(image_filename_mod),
                         "%s/%s", mod_data_path, image_filename);
...
void *load_from_opendats_alloc(int resource, const char *extension,
  ...
  int size;
  FILE *fp = NULL;
  load_from_opendats_metadata(resource, extension, &fp, &result, &checksum,
                              &size, &pointer);
  if (out_result != NULL)
    *out_result = result;
  if (out_size != NULL)
  ...
  if (fread(area, size, 1, fp) != 1) {
    fprintf(stderr, "%s: %s, resource %d, size %d, failed: %s\n", __func__,
            pointer->filename, resource, size, strerror(errno));
    free(area);
    area = NULL;
  }
...
// seg009:A172
int load_from_opendats_to_area(int resource, void *area, int length,
                               const char *extension) {
  // stub
  // return 0;
  dat_type *pointer;
...
void method_1_blit_rect(surface_type *target_surface,
                        surface_type *source_surface,
                        const rect_type *target_rect,
                        const rect_type *source_rect, int blit) {
  SDL_Rect src_rect;
...
image_type *method_3_blit_mono(image_type *image, int xpos, int ypos,
                               int blitter, byte color) {
  int w = image->w;
  int h = image->h;
  if (SDL_SetColorKey(image, SDL_TRUE, 0) != 0) {
  ...
  */
  ...
  uint32_t rgb_color = SDL_MapRGB(colored_image->format, palette_color.r << 2,
                                  palette_color.g << 2, palette_color.b << 2) &
                       0xFFFFFF;
  int stride = colored_image->pitch;
  for (int y = 0; y < h; ++y) {
    uint32_t *pixel_ptr =
        (uint32_t *)((byte *)colored_image->pixels + stride * y);
    for (int x = 0; x < w; ++x) {
  ...
  if (SDL_BlitSurface(colored_image, &src_rect, current_target_surface,
                      &dest_rect) != 0) {
...
bool RGB24_bug_check(void) {
  if (!RGB24_bug_checked) {
    ...
    if (NULL == test_surface)
      sdlperror("SDL_CreateSurface in RGB24_bug_check");
    // Fill with red.
    SDL_FillRect(test_surface, NULL,
                 SDL_MapRGB(test_surface->format, 0xFF, 0, 0));
    if (0 != SDL_LockSurface(test_surface))
      sdlperror("SDL_LockSurface in RGB24_bug_check");
    // Read red component of pixel.
    RGB24_bug_affected =
        (*(Uint32 *)test_surface->pixels & test_surface->format->Rmask) == 0;
...
int safe_SDL_FillRect(SDL_Surface *dst, const SDL_Rect *rect, Uint32 color) {
  if (dst->format->BitsPerPixel == 24 && RGB24_bug_check()) {
    // In the buggy version, SDL_FillRect swaps R and B, so we swap it once
    // more.
    color =
        ((color & 0xFF) << 16) | (color & 0xFF00) | ((color & 0xFF0000) >> 16);
  }
...
#ifndef USE_ALPHA
  uint32_t rgb_color =
      SDL_MapRGBA(current_target_surface->format, palette_color.r << 2,
                  palette_color.g << 2, palette_color.b << 2, 0xFF);
#else
  uint32_t rgb_color =
      SDL_MapRGBA(current_target_surface->format, palette_color.r << 2,
                  palette_color.g << 2, palette_color.b << 2,
                  color == 0 ? SDL_ALPHA_TRANSPARENT : SDL_ALPHA_OPAQUE);
#endif
...
void draw_rect_with_alpha(const rect_type *rect, byte color, byte alpha) {
  ...
  rgb_type palette_color = palette[color];
  uint32_t rgb_color =
      SDL_MapRGBA(overlay_surface->format, palette_color.r << 2,
                  palette_color.g << 2, palette_color.b << 2, alpha);
  if (safe_SDL_FillRect(current_target_surface, &dest_rect, rgb_color) != 0) {
...
void blit_xor(SDL_Surface *target_surface, SDL_Rect *dest_rect,
              SDL_Surface *image, SDL_Rect *src_rect) {
  if (dest_rect->w != src_rect->w || dest_rect->h != src_rect->h) {
    printf("blit_xor: dest_rect and src_rect have different sizes\n");
    quit(1);
  }
  SDL_Surface *helper_surface = SDL_CreateRGBSurface(
      0, dest_rect->w, dest_rect->h, 24, Rmsk, Gmsk, Bmsk, 0);
...
void draw_colored_torch(int color, SDL_Surface *image, int xpos, int ypos) {
  ...
  uint32_t old_color = SDL_MapRGB(colored_image->format, 0xFC, 0x84, 0x00) &
                       0xFFFFFF; // the orange in the flame
  uint32_t new_color =
      SDL_MapRGB(colored_image->format, iRed, iGreen, iBlue) & 0xFFFFFF;
  int stride = colored_image->pitch;
  for (int y = 0; y < h; ++y) {
    uint32_t *pixel_ptr =
        (uint32_t *)((byte *)colored_image->pixels + stride * y);
...
image_type *method_6_blit_img_to_scr(image_type *image, int xpos, int ypos,
                                     int blit) {
  if (image == NULL) {
  ...
  //  Fix the background color of teleport images on SDL_image 2.6.2, where they
  //  are loaded as RGBA. For transparency, paletted images need colorkeying,
  //  RGB(A) images need blending.
  if (blit == blitters_0_no_transp) {
...
void recalculate_feather_fall_timer(double previous_ticks_per_second,
  ...
  is_feather_fall =
      is_feather_fall / previous_ticks_per_second * ticks_per_second;
}
...
void set_timer_length(int timer_index, int length) {
  ...
  if (is_feather_fall == 0 || wait_time[timer_index] < custom->base_speed ||
...
void process_events() {
  ...
  // Previously, this procedure would wait for *one* event and process it, then
  // return. Much like the x86 HLT instruction. (We still want to process all
  // events in the queue. For instance, there might be simultaneous SDL2 KEYDOWN
  // and TEXTINPUT events.)
  SDL_Event event;
  while (SDL_PollEvent(&event) ==
         1) { // while there are still events to be processed
    switch (event.type) {
    case SDL_KEYDOWN: {
      int modifier = event.key.keysym.mod;
      int scancode = event.key.keysym.scancode;

      // Handle these separately, so they won't interrupt things that are
      // usually interrupted by a keypress. (pause, cutscene)
#ifdef USE_FAST_FORWARD
      ...
      if (scancode == SDL_SCANCODE_F12) {
        ...
        break; // Prevent repeated keystrokes opening/closing the menu as long
               // as the key is held down.
      } else
#endif
          if ((modifier & KMOD_ALT) && scancode == SDL_SCANCODE_RETURN) {
        ...
        default:
          ...
          if (modifier & KMOD_SHIFT)
            last_key_scancode |= WITH_SHIFT;
          if (modifier & KMOD_CTRL)
            last_key_scancode |= WITH_CTRL;
          if (modifier & KMOD_ALT)
            last_key_scancode |= WITH_ALT;
        }
    ...
    case SDL_KEYUP:
      // If Alt was held down from Alt+Tab but now it's released: stop ignoring
      // Tab.
      if (event.key.keysym.scancode == SDL_SCANCODE_TAB && ignore_tab)
        ignore_tab = false;
...
#ifdef USE_MENU
      // Prevent repeated keystrokes opening/closing the menu as long as the key
      // is held down.
...
#ifdef USE_AUTO_INPUT_MODE
        if (!is_joyst_mode && (event.caxis.value >= joystick_threshold ||
                               event.caxis.value <= -joystick_threshold)) {
          is_joyst_mode = 1;
          is_keyboard_mode = 0;
        }
#endif
    ...
    case SDL_CONTROLLERBUTTONDOWN:
      ...
      case SDL_CONTROLLER_BUTTON_DPAD_LEFT:
        joy_button_states[JOYINPUT_DPAD_LEFT] |=
            KEYSTATE_HELD | KEYSTATE_HELD_NEW;
        break; // left
      case SDL_CONTROLLER_BUTTON_DPAD_RIGHT:
        joy_button_states[JOYINPUT_DPAD_RIGHT] |=
            KEYSTATE_HELD | KEYSTATE_HELD_NEW;
        break; // right
      case SDL_CONTROLLER_BUTTON_DPAD_UP:
        joy_button_states[JOYINPUT_DPAD_UP] |=
            KEYSTATE_HELD | KEYSTATE_HELD_NEW;
        break; // up
      case SDL_CONTROLLER_BUTTON_DPAD_DOWN:
        joy_button_states[JOYINPUT_DPAD_DOWN] |=
            KEYSTATE_HELD | KEYSTATE_HELD_NEW;
        break; // down
      ...
      case SDL_CONTROLLER_BUTTON_BACK:
        if (event.cbutton.button == SDL_CONTROLLER_BUTTON_START)
          joy_button_states[JOYINPUT_START] |=
              KEYSTATE_HELD | KEYSTATE_HELD_NEW;
        else if (event.cbutton.button == SDL_CONTROLLER_BUTTON_BACK)
          joy_button_states[JOYINPUT_BACK] |= KEYSTATE_HELD | KEYSTATE_HELD_NEW;
#ifdef USE_MENU
        last_key_scancode =
            SDL_SCANCODE_BACKSPACE; /*** bring up pause menu ***/
#else
    ...
    case SDL_JOYAXISMOTION:
      // Only handle the event if the joystick is incompatible with the
      // SDL_GameController interface. (Otherwise it will interfere with the
      // normal action of the SDL_GameController API.)
      ...
      if (event.type == SDL_JOYAXISMOTION) {
        ...
        if ((dword)(joy_x * joy_x) + (dword)(joy_y * joy_y) <
            (dword)(joystick_threshold * joystick_threshold)) {
          break;
        }
      }
#ifdef USE_AUTO_INPUT_MODE
      ...
      if (event.type == SDL_JOYBUTTONDOWN) {
        if (event.jbutton.button == SDL_JOYSTICK_BUTTON_Y)
          joy_button_states[JOYINPUT_Y] |=
              KEYSTATE_HELD | KEYSTATE_HELD_NEW; // Y (up)
        else if (event.jbutton.button == SDL_JOYSTICK_BUTTON_X)
          joy_button_states[JOYINPUT_X] |=
              KEYSTATE_HELD | KEYSTATE_HELD_NEW; // X (Shift)
      } else if (event.type == SDL_JOYBUTTONUP) {
        if (event.jbutton.button == SDL_JOYSTICK_BUTTON_Y)
          joy_button_states[JOYINPUT_Y] &= ~KEYSTATE_HELD; // Y (up)
        else if (event.jbutton.button == SDL_JOYSTICK_BUTTON_X)
          joy_button_states[JOYINPUT_X] &= ~KEYSTATE_HELD; // X (Shift)
      }
      break;
    ...
    case SDL_TEXTINPUT:
      ...
      // because event.key.keysym.sym only tells us what character would the key
      // type without shift.
    ...
    case SDL_WINDOWEVENT:
      // In case the user switches away while holding a key: do as if all keys
      // were released. (DOSBox does the same.)

      /* // not implemented in SDL2 for now
       *
                              if ((event.active.state & SDL_APPINPUTFOCUS) &&
       event.active.gain == 0) { memset(key_states, 0, sizeof(key_states));
                              }
                              // Note: event.active.state can contain multiple
       flags or'ed.
                              // If the game is in full screen, and I switch
       away (Alt+Tab) and back, most of the screen will be black, until it is
       redrawn. if ((event.active.state & SDL_APPACTIVE) && event.active.gain ==
       1) { update_screen();
                              }
      */
      ...
      case SDL_WINDOWEVENT_FOCUS_GAINED:
        // Fix for this bug: When playing back a recording, Alt+Tabbing back to
        // SDLPoP stops the replay if Alt is released before Tab.
        { // If Alt is held down from Alt+Tab: ignore it until it's released.
    ...
    case SDL_USEREVENT:
      if (event.user.code ==
          userevent_TIMER /*&& event.user.data1 == (void*)timer_index*/) {
#ifdef USE_COMPAT_TIMER
    ...
    case SDL_MOUSEBUTTONDOWN:
      ...
      case SDL_BUTTON_X1: // 'Back' button (on mice that have these extra
                          // buttons).
...
#ifdef USE_REPLAY
  if ((replaying && skipping_replay) || is_validate_mode)
    return;
#endif
  update_screen();
...
// seg009:365C
void reset_clip_rect() { SDL_SetClipRect(current_target_surface, NULL); }
...
void set_bg_attr(int vga_pal_index, int hc_pal_index) {
  ...
  if (vga_pal_index == 0) {
    ...
    */
    ...
    if (SDL_SetColorKey(offscreen_surface, SDL_TRUE, 0) !=
        0) { // SDL_SRCCOLORKEY old
    ...
    uint32_t rgb_color =
        SDL_MapRGB(onscreen_surface_->format, palette_color.r << 2,
                   palette_color.g << 2, palette_color.b << 2) /*& 0xFFFFFF*/;
...
// seg009:07EB
rect_type *offset4_rect_add(rect_type *dest, const rect_type *source,
...
// seg009:3AA5
rect_type *offset2_rect(rect_type *dest, const rect_type *source, int delta_x,
                        int delta_y) {
...
// seg009:1A51
palette_fade_type *make_pal_buffer_fadein(surface_type *source_surface,
                                          int which_rows, int wait_time) {
  palette_fade_type *palette_buffer;
  ...
  memcpy(palette_buffer->faded_pal, palette_buffer->original_pal,
         sizeof(palette_buffer->faded_pal));
  for (word curr_row = 0, curr_row_mask = 1; curr_row < 0x10;
       ++curr_row, curr_row_mask <<= 1) {
    if (which_rows & curr_row_mask) {
      memset(palette_buffer->faded_pal + (curr_row << 4), 0,
             sizeof(rgb_type[0x10]));
...
void pal_restore_free_fadein(palette_fade_type *palette_buffer) {
  ...
  free(palette_buffer);
  // for RGB
  method_1_blit_rect(onscreen_surface_, offscreen_surface, &screen_rect,
                     &screen_rect, 0);
}
...
int fade_in_frame(palette_fade_type *palette_buffer) {
  ...
  for (word start = 0, current_row_mask = 1; start < 0x100;
       start += 0x10, current_row_mask <<= 1) {
...
void set_chtab_palette(chtab_type *chtab, byte *colors, int n_colors) {
  if (chtab != NULL) {
    ...
    for (int i = 0; i < n_colors; ++i) {
      ...
      ++colors;
      scolors[i].a =
          SDL_ALPHA_OPAQUE; // the SDL2 SDL_Color struct has an alpha component
    }

    // Color 0 of the palette data is not used, it is replaced by the background
    // color. Needed for correct alternate colors (v1.3) of level 8.
    ...
    for (int i = 0; i < chtab->n_images; ++i) {
      ...
      if (current_image != NULL) {
        ...
        if (current_palette != NULL) {
          ...
          // this caused SDL_SetPaletteColors to fail, I think because that
          // palette contains only 2 colors
...
int has_timer_stopped(int timer_index) {
  ...
  int ticks_elapsed =
      (int)((current_counter / perf_counters_per_tick) -
            (timer_last_counter[timer_index] / perf_counters_per_tick));
  int overshoot = ticks_elapsed - wait_time[timer_index];
  if (overshoot >= 0) {
    //		float milliseconds_elapsed = (current_counter -
    // timer_last_counter[timer_index]) * milliseconds_per_counter;
    // printf("timer %d:   frametime (ms) = %5.1f    fps = %.1f    timer ticks
    // elapsed = %d\n", timer_index, milliseconds_elapsed, 1000.0f /
    // milliseconds_elapsed, ticks_elapsed);
