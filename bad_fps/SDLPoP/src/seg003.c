...
int RL_Mode = 0;
int action = 0;
int prince_death = 0; // set to 1 by C when Kid dies, Python resets it
int RL_Visual = 0;    // set to 1 by Python for visual mode (renders each frame)
// seg003:0000
void init_game(int level) {
  ...
  if (RL_Mode) {
    rem_min = -1; // RL: disable game timer (negative = no expiry)
    rem_tick = 0;
  }
...
void play_level(int level_number) {
  ...
  for (;;) {
    ...
    if (level_number != current_level) {
      ...
      if (cutscene_func != NULL && !RL_Mode // RL - no cutscenes
#ifdef USE_REPLAY
          && !(recording || replaying)
#endif
    ...
    if (RL_Mode)
      return; // RL: exit after 1 frame, pop_main() returns to Python
              // hacked...
#ifdef USE_COPYPROT
    if (enable_copyprot && level_number == custom->copyprot_level &&
        !demo_mode) {
