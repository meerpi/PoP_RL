...
void get_row_collision_data(short row, sbyte *row_coll_room_ptr,
...
short trigger_1(short target_type, short room, short tilepos,
                short button_type);
...
int get_tile_to_draw(int room, int column, int row, byte *ptr_tile,
                     byte *ptr_modifier, byte tile_room0);
...
int add_backtable(short chtab_id, int id, sbyte xh, sbyte xl, int ybottom,
                  int blit, byte peel);
int add_foretable(short chtab_id, int id, sbyte xh, sbyte xl, int ybottom,
                  int blit, byte peel);
int add_midtable(short chtab_id, int id, sbyte xh, sbyte xl, int ybottom,
                 int blit, byte peel);
void add_peel(int left, int right, int top, int height);
void add_wipetable(sbyte layer, short left, short bottom, sbyte height,
                   short width, sbyte color);
...
void draw_image(image_type *image, image_type *mask, int xpos, int ypos,
                int blit);
...
#define locate_file(filename)                                                  \
  locate_file_(filename, alloca(POP_MAX_PATH), POP_MAX_PATH)
#define locate_save_file(filename)                                             \
  locate_save_file_(filename, alloca(POP_MAX_PATH), POP_MAX_PATH)
const char *locate_file_(const char *filename, char *path_buffer,
                         int buffer_size);
const char *locate_save_file_(const char *filename, char *path_buffer,
                              int buffer_size);
...
// We define a function-like macro, because `stat` is also the name of the type,
// and we don't want to redefine that.
...
// RL globals (defined in seg003.c)
extern int RL_Mode;
extern int action;
extern int prince_death;
extern int RL_Visual;
