...
int has_timer_stopped(int timer_index) {
	...
	int effective_wait = wait_time[timer_index];
	if (rl_speed_multiplier > 1)
		effective_wait = (effective_wait > 0) ? MAX(1, effective_wait / rl_speed_multiplier) : 0;
	int ticks_elapsed = (int)((current_counter / perf_counters_per_tick) - (timer_last_counter[timer_index] / perf_counters_per_tick));
	int overshoot = ticks_elapsed - effective_wait;
	if (overshoot >= 0) {
