...
void play_level(int level_number) {
	...
	for (;;) {
		...
		if (level_number != current_level) {
			...
			if (cutscene_func != NULL && !RL_state
...
int play_level_2() {
	...
	while (1) { // main loop
		...
		timers();
		if (RL_state)
			rl_pre_frame(); /* gate: block until Python authorizes this frame */
		play_frame();
		pop_frame_counter++;
		if (RL_state)
			rl_frame_hook();
		...
		if (is_restart_level) {
			...
			if (next_level == current_level || (check_sound_playing() && !RL_state)) {
				if (!RL_state || !rl_headless) {
					draw_game_frame();
					flash_if_hurt();
